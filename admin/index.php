    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDrop: function(info) {
          if (info.event.extendedProps.source === 'project_tasks') {
            Swal.fire({
              title: '<h2 style="font-size: 24px;">คำเตือน</h2>',
              html: '<p style="font-size: 18px;">ไม่สามารถเปลี่ยนวันที่ของโครงการได้ กรุณาไปแก้ไขที่หน้า <b>"จัดการโครงการ"</b></p>',
              icon: 'warning',
      ...
      function showEventDetails(event) {
        ...
        let isProjectTask = event.extendedProps.source === "project_tasks";

        let buttons = isProjectTask ? {
          confirmButtonText: "ปิด",
          showCancelButton: true,
          cancelButtonText: "แก้ไขโครงการ",
        } : {
          confirmButtonText: "ปิด",
          showDenyButton: true,
          denyButtonText: "ลบกิจกรรม",
        };
        ...
        Swal.fire({
          ...
          } else if (result.dismiss === Swal.DismissReason.cancel && isProjectTask) {
            window.location.href = `project.php?act=edit&ID=${event.id}`;
          }
        });
      }
        ...
        success: function(response) {
          let formattedEvents = response.map(event => {
            ...
            return {
              ...
              owner_name: event.owner_name || "ไม่ระบุ"
            };
          });
